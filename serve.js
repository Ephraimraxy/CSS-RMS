...
app.post('/api/onboarding/bulk-sms', authenticateToken, requireRoles(['global_admin']), batchUpload.single('file'), async (req, res) => {
  try {
    ...
    for (const row of rows) {
      ...
      const message = `Dear ${displayName}, welcome to CSS Group! Your official email is: ${email}. Default password: ${defaultPwd}. Login at webmail.cssgroup.com.ng and change your password immediately after first login.${dept ? ` Dept: ${dept}.` : ''}${position ? ` Role: ${position}.` : ''} - CSS Group Admin`;
